...
from datatuner.classification.consistency_processor import ConsistencyProcessor
...
def write_args_to_file(args, filename):
    ...
    json.dump(args_dict, open(filename, "w"), indent=2)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        args.passed_examples = hasattr(args, "examples")
        if args.passed_examples:
        ...
        if args.max_data > 0:
            examples = examples[: args.max_data]
        ...
        if args.filter_long_seq:
            print(f"original number of examples: {len(features)}")
            features = [x for x in features if x.input_ids[-1] == 1]
            print(f"after filtering, the number of examples is: {len(features)}")
        if not args.passed_examples and args.local_rank in [-1, 0]:
...
def main(args_dict=None):
    ...
    parser.add_argument(
        "--filter_long_seq",
        action="store_true",
        help="Whether to filter out sequences longer than tokenizer supported sequence length.",
    )
    ...
    parser.add_argument(
        "--xrt_tpu_config", type=str, default="", help="XRT TPU config if none are set in the environment variables"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="JSON file with training parameters; used for retraining based on these paramters",
    )
    ...
    if args_dict is not None:
        for arg in args_dict:
            args.__setattr__(arg, args_dict[arg])

    if (
        (args.do_train or args.do_eval)
